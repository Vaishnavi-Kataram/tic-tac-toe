...
ICON_X = load_icon('x-icon.png', [PIXEL_WIDTH-40, PIXEL_WIDTH-40])
ICON_O = load_icon('o-icon.png', [PIXEL_WIDTH-40, PIXEL_WIDTH-40])
...
def draw_icons():
    for i, row in enumerate(board):
        for j, col in enumerate(board[i]):
            ...
            cell_center_x = j * PIXEL_WIDTH + PIXEL_WIDTH // 2
            cell_center_y = i * PIXEL_WIDTH + PIXEL_WIDTH // 2

            if board[i][j] == 0:
                # Calculate the top-left corner coordinates for blitting the image
                image_x = cell_center_x - ICON_O.get_width() // 2
                image_y = cell_center_y - ICON_O.get_height() // 2
                screen.blit(ICON_O, (image_x, image_y))

            if board[i][j] == 1:
                # Calculate the top-left corner coordinates for blitting the image
                image_x = cell_center_x - ICON_X.get_width() // 2
                image_y = cell_center_y - ICON_X.get_height() // 2
                screen.blit(ICON_X, (image_x, image_y))
